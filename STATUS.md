...
5. 校徽換成國立臺灣師範大學校徽、DSH 網頁介面（`@deepseek-ai/dsh`）自動轉換成台灣繁體用詞——已在正式 Proxmox VM 上重建 image 並確認瀏覽器上實際顯示正確。

**已知、尚未驗證的規模假設**：原始規劃是抓 10 人（尖峰 15 人）同時使用估算 VM 規格（見 README「VM 規格建議」），但目前只有 2 個帳號先後登入過，還沒有真正多人同時在線的實測；資源限制的數字本身是對的，但併發情境下 VM 負載、排隊機制的實際表現還沒有機會驗證，不算缺陷，只是還沒測到這個規模，之後使用量上來後值得留意。
